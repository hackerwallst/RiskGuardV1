...
def _deals_for_position(reader: RiskGuardMT5Reader, position_ticket: int,
                        since_epoch: Optional[int]) -> List[Any]:
    try:
...
def _summarize_closed_position(reader: RiskGuardMT5Reader, pos_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    deals = _deals_for_position(reader, ticket, since_epoch=pos_state.get("open_time_epoch"))
    if not deals:
        return {"ok": False, "reason": "Sem histórico de deals.", "ticket": ticket}

    try:
        import MetaTrader5 as mt5
        out_entries = {getattr(mt5, "DEAL_ENTRY_OUT", 1), getattr(mt5, "DEAL_ENTRY_INOUT", 2)}
    except Exception:
        out_entries = {1, 2}
